 ...
 * aujourd'hui) → le panneau affiche « à jour ». D'où la garde sur la carte vide.
 */
export function useRevision(): DueCounts | null {
  return useClientOnce(() => {
    const map = asFsrs(readRawProgress());
    if (!Object.keys(map).length) return null; // rien collecté encore → état vide
    return dueBySkill(map, dayNumber(new Date()));
  });
}

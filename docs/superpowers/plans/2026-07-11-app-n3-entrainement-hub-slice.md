...
- **D5 — `dict.js` non chargé sur les pages React (bug préexistant, découvert à la vérif Task 8).** Le bundler HTML de `bun build` **supprime `<script src="dict.js">`** des pages React (`quiz.html` ET `app-n3.html`) — `dict.js` est un IIFE classique (pas un module ES), donc bun retire la balise. Le fichier reste copié dans `_site` et sa balise survit sur les pages vanilla `cours-n3`/`planning-n3` (qui l'utilisent), mais sur les pages React `window.initDefs` est `undefined` en prod. **Impact hub : nul** (aucun contenu japonais à taper sur le hub ; `initDefs?.()` = no-op sûr). **Impact quiz : le tap-pour-définir/furigana est cassé en prod** — bug latent de la tranche quiz, hors périmètre de cette tranche. Correctif futur : importer `dict.js` pour effet de bord depuis l'entrée TSX (`import "../../dict.js"` — l'IIFE s'exécute et pose `window.initDefs`), OU convertir `dict.js` en module et l'importer. À traiter dans une tranche dédiée (touche `quiz.tsx`, nécessite ses propres tests).

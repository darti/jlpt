...
La couverture kanji est passée de 54 % à 67 % avec l'ajout du **15ᵉ type
`kanji-hors-contexte`** (« erreur : le kanji « X » ne convient pas ici », 1 222 notes) : un piège
fréquent — le kanji existe et se lit bien, mais ne correspond pas au sens de la phrase — jusque-là
noyé dans `autre`. Le motif est `/ne convient pas/`, mesuré propre (1 222 notes, toutes en `autre`
avant, aucun vol d'un autre type, aucune coexistant avec un marqueur de lecture/sens). Le reste en
`autre` (≈ 33 %) est de la confusion de *sens* ou des gloses nues, sans type dédié — honnêtement
non classé plutôt que faussement typé.
...
Quatorze types, tous issus des formules réellement employées par l'auteur dans les notes (les
« parts mesurées » datent du typage initial ; les motifs ont depuis été resserrés puis complétés
du 15ᵉ type — cf. § 1) :

| Type | Formule d'origine |
|---|---|
| `lecture-erronee` | « lecture erronée », « erreur : … se lit … » |
| `lecture-on-kun` | « lecture on », « on'yomi », « kun » |
| `graphie-inexistante` | « graphie inexistante », « n'existe pas » |
| `kanji-partage` | « partage 響 », « même kanji » |
| `voisement` | « voisement erroné », « assimilation phonétique » |
| `lecture-autre-mot` | « lecture de 自由, pas de 理由 » |
| `forme-proche` | « forme proche », « ressemble » |
| `kanji-confondu` | « confond avec 幹 » |
| `kanji-hors-contexte` | « le kanji « X » ne convient pas ici » |
| `longueur-voyelle` | « voyelle manquante », « gémination », « son long » |
| `sens-different` | « sens différent », « autre mot », « sens de » |
| `homophone` | « homophone », « même lecture » |
| `nuance-grammaticale` | « exprime un but, pas la simultanéité » |
| `registre` | « registre », « poli / neutre » |
| `autre` | *aucun motif ne matche* |
